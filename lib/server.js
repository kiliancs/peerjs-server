...
app._configureWS = function(socket, key, id, token) {
  ...
  // disconnects.
  this.emit('connection', id, key);
};

app._validateKey = function(key) {
  return  key == this._options.key ||
          (
            Object.prototype.toString.call( this._options.key ) === '[object Array]' &&
            this._options.key.indexOf(key) != -1
          );
};
